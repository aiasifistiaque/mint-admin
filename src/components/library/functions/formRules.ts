 ...
 *
 * The admin's copy of the backend's library/functions/formRules.function.ts
 * (without the Joi schema and the route helpers' server use) — keep the two in step.
 */
...
/** A form's rules, from its fields' `renderIf`. */
export const rulesOfFields = (fields: any[] | undefined): Rules => {
	const out: Rules = {};
	for (const f of fields || []) if (f?.name && f?.renderIf && typeof f.renderIf === 'object') out[f.name] = f.renderIf;
	return out;
};

/** The fields a form hides for these values — also what a submit leaves out. */
export const hiddenFormFields = (fields: any[] | undefined, values: any): Set<string> =>
	new Set(hiddenFields(rulesOfFields(fields), values || {}));

/** A payload without the fields its form hides. */
export const withoutHidden = (body: any, fields: any[] | undefined, values?: any) => {
	const hidden = hiddenFormFields(fields, values ?? body);
	if (!hidden.size || !body || typeof body !== 'object') return body;
	const out = { ...body };
